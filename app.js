...
require([
    ...
    // ==========================================================================
    let state = {
        mesas: [],          // Mesas en memoria; la fuente de verdad es el Feature Server
        colegiosCerrados: [],
    ...
    function isConfigRecordCodigo(cod) {
        return cod === "__PARTIES__" || cod === "__CLOSED__";
    }

    function isScrutinyFormActive() {
        return !!(state.selectedMesa);
    }

    function initLocalDatabase() {
        ["elecciones_mesas", "elecciones_parties_config", "elecciones_colegios_cerrados", "elecciones_censos_manuales"].forEach(k => {
            try { localStorage.removeItem(k); } catch (e) {}
        });
        state.mesas = [];
        state.colegiosCerrados = [];
    ...
    function setupEventListeners() {
        ...
        if (inputP1) {
            ...
            inputP1.addEventListener("keydown", blockNegativeNumberKeys);
        }
        const inputP2 = document.getElementById("input-part2-voters");
        if (inputP2) {
            inputP2.addEventListener("input", updateAdvance2LivePercent);
            inputP2.addEventListener("keydown", blockNegativeNumberKeys);
        }
    ...
    function blockNegativeNumberKeys(e) {
        if (e.key === "-" || e.key === "Minus" || e.key === "e" || e.key === "E" || e.key === "+" || e.key === ".") {
            e.preventDefault();
        }
    }

    function sanitizeNonNegativeIntegerInput(input) {
        if (!input) return 0;
        let rawStr = String(input.value || "");
        rawStr = rawStr.replace(/[^0-9]/g, "");
        if (rawStr.length > 1 && /^0+[0-9]+/.test(rawStr)) {
            rawStr = rawStr.replace(/^0+/, "");
        }
        if (input.value !== rawStr) {
            input.value = rawStr;
        }
        if (rawStr === "") return 0;
        return Math.max(0, parseInt(rawStr, 10) || 0);
    }

    function updateAdvanceLivePercent(phase) {
        if (!state.selectedMesa) return;
        const isPart2 = phase === "part2";
        const input = document.getElementById(isPart2 ? "input-part2-voters" : "input-part1-voters");
        const valEl = document.getElementById(isPart2 ? "part2-live-percent-val" : "part1-live-percent-val");
        const subEl = document.getElementById(isPart2 ? "part2-live-ratio-sub" : "part1-live-ratio-sub");
        const badge = document.getElementById(isPart2 ? "part2-live-percent-badge" : "part1-live-percent-badge");
        const censo = parseInt(state.selectedMesa.censo, 10) || 0;
        const val = sanitizeNonNegativeIntegerInput(input);
        const overCensus = censo > 0 && val > censo;
        const pctNum = censo > 0 ? (val / censo) * 100 : 0;
        const pct = pctNum.toFixed(2);

        if (valEl) valEl.textContent = `${pct}%`;
        if (subEl) {
            subEl.textContent = overCensus
                ? `Supera el censo (${val.toLocaleString()} de ${censo.toLocaleString()} electores)`
                : `${val.toLocaleString()} de ${censo.toLocaleString()} electores`;
        }
        if (badge) {
            badge.classList.toggle("over-census", overCensus);
        }
    }

    function updateAdvance1LivePercent() {
        updateAdvanceLivePercent("part1");
    }

    function updateAdvance2LivePercent() {
        updateAdvanceLivePercent("part2");
    }
    ...
    function savePartiesToStorage(newParties) {
        ...
        console.log("[PARTIDOS] Configuración en memoria (GDB):", PARTIES_CONFIG.length, "partidos");
    }
    ...
    function generateVoteFields(forceRebuild) {
        const parties = getPartiesConfig();
        const container = document.getElementById("portal-votes-fields-container");
        if (!container) return;
        if (!forceRebuild && isScrutinyFormActive() && container.querySelector(".vote-input-field")) {
            return;
        }
        container.innerHTML = "";
    ...
    function renderAdminColegiosList() {
        ...
        colegios.forEach(colName => {
            ...
            const isAllClosed = (closed === total && total > 0) || (state.colegiosCerrados && state.colegiosCerrados.includes(colName));
    ...
    function handleAdminCloseSchool(colegioName) {
        ...
        if (!state.colegiosCerrados.includes(colegioName)) {
            state.colegiosCerrados.push(colegioName);
        }
        saveClosedSchoolsToArcGIS(state.colegiosCerrados);
        renderAdminPortal();
        updateGlobalMetrics();
    }
    ...
    function handleAdminDeleteParty(partyId) {
        ...
        savePartiesToStorage(updated);
        savePartiesToArcGIS(PARTIES_CONFIG);
...
// DESCARGA Y SINCRONIZACIÓN CON ARCGIS FEATURE SERVER
    ...
    function applyElectoralConfigFromFeatures(features) {
        if (!features || !features.length) return;
        features.forEach(feat => {
            const attrs = feat.attributes || feat;
            const cod = getAttributeValue(attrs, "codigo");
            const raw = getAttributeValue(attrs, "RESULTADOS_JSON") || getAttributeValue(attrs, "resultados_json");
            if (cod === "__PARTIES__") {
                let serverList = [];
                try {
                    if (raw) {
                        const parsed = typeof raw === "string" ? JSON.parse(raw) : raw;
                        if (parsed && Array.isArray(parsed.parties_config)) {
                            serverList = parsed.parties_config;
                        } else if (Array.isArray(parsed)) {
                            serverList = parsed;
                        }
                    }
                } catch (e) {
                    console.warn("[PARTIDOS] Error leyendo __PARTIES__ de la GDB:", e);
                }
                savePartiesToStorage(serverList);
                const partySig = PARTIES_CONFIG.map(p => p.id).sort().join(",");
                if (!isScrutinyFormActive()) {
                    generateVoteFields();
                }
                console.log("[PARTIDOS] Aplicados desde GDB:", serverList.length, "partidos", partySig);
            }
            if (cod === "__CLOSED__") {
                try {
                    const parsed = raw ? (typeof raw === "string" ? JSON.parse(raw) : raw) : {};
                    state.colegiosCerrados = Array.isArray(parsed.closed) ? parsed.closed : [];
                } catch (e) {
                    state.colegiosCerrados = [];
                }
            }
        ...
        try {
            ...
            tablesLayer.load().then(() => {
                ...
                tablesLayer.queryFeatures(query).then(results => {
                    ...
                    if (results.features && results.features.length > 0) {
                        applyElectoralConfigFromFeatures(results.features);

                        const serverMesas = [];
                        results.features.forEach(feat => {
                            const cod = getAttributeValue(feat.attributes, "codigo");
                            if (!cod || isConfigRecordCodigo(cod)) return;
    ...
    function syncDataWithArcGISServer() {
        ...
        if (isScrutinyFormActive()) return;
    ...
    function saveClosedSchoolsToArcGIS(closedList) {
        if (typeof FeatureLayer === "undefined" || !URL_MESAS_TABLE_EDIT || !state.arcgisMode) return;
        const list = Array.isArray(closedList) ? closedList : [];
        try {
            const configJson = JSON.stringify({ closed: list });
            const attrs = {
                CODIGO: "__CLOSED__",
                SECCION: "0",
                MESA: "CONFIG",
                ...
                ESTADO: "Config",
                RESULTADOS_JSON: configJson,
                MIEMBROS: list.length.toString() + " colegios"
            };
            const tablesLayer = new FeatureLayer({ url: URL_MESAS_TABLE_EDIT, outFields: ["*"] });
            const q = tablesLayer.createQuery();
            q.where = "CODIGO = '__CLOSED__'";
            q.outFields = ["OBJECTID"];
            tablesLayer.queryFeatures(q).then(res => {
                if (res.features && res.features.length > 0) {
                    attrs.OBJECTID = res.features[0].attributes.OBJECTID;
                    return tablesLayer.applyEdits({ updateFeatures: [new Graphic({ attributes: attrs })] });
                }
                return tablesLayer.applyEdits({ addFeatures: [new Graphic({ attributes: attrs })] });
            }).then(result => {
                console.log("[COLEGIOS] Cierres guardados en ArcGIS:", list.length, result);
            }).catch(err => {
                console.warn("[COLEGIOS] Error al guardar cierres en ArcGIS:", err);
            });
        } catch (e) {
            console.warn("[COLEGIOS] Excepción en saveClosedSchoolsToArcGIS:", e);
        }
    }
